...
export default function ReapproScreen({ route }) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const baseProducts = mode === 'returns'
      ? products.filter((product) => {
        ...
        return line && (
          toNumber(line.qty_loaded) > 0
          || toNumber(line.qty_sold) > 0
          || toNumber(line.qty_returned) > 0
        )
      })
      : products
  ...
  }, [lineByProductId, mode, products, search])
  ...
  const submit = async () => {
    ...
    if (payload.length === 0) {
      Alert.alert(
        ...
        mode === 'returns'
          ? 'Ajoutez au moins une quantite retour.'
          : 'Ajoutez au moins une quantite a charger.',
      )
      return
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        error.response?.data?.message || error.message || 'Veuillez reessayer.',
      )
    }
  }
  ...
  return (
    ...
    >
      <PageHeader
        ...
        subtitle="Depot, retours et ecarts relies a la session mobile."
      ...
      {!session || !isOpen ? (
        <View style={[s.emptyCard, cardShadow]}>
          ...
          <Text style={s.emptyText}>
            Ouvrez d abord la session du jour et affectez le camion physique pour declarer les chargements et retours.
          </Text>
          <TouchableOpacity style={s.primaryButton} onPress={() => navigation.navigate('Tabs', { screen: 'Session' })}>
            <Text style={s.primaryButtonText}>Ouvrir Session & GPS</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <>
          <View style={[s.heroCard, cardShadow]}>
            <View style={s.heroTop}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.heroSubtitle}>
                  {configuredCamion?.name
                    ? `${configuredCamion.name}${configuredCamion?.plate ? ` · ${configuredCamion.plate}` : ''}`
            ...
            <View style={s.factGrid}>
              ...
              <View style={s.factItem}>
                ...
                <Text style={s.factValue}>{formatNumber((session.lines ?? []).length)}</Text>
              </View>
              <View style={s.factItem}>
            ...
            <View style={s.noticeCard}>
              ...
              <Text style={s.noticeText}>
                Chaque validation met a jour la session mobile, l audit terrain et les notifications cote plateforme.
              </Text>
            </View>
          </View>
          ...
          <View style={[s.sectionCard, cardShadow]}>
            <View style={s.segmentedWrap}>
              ...
              >
                <MaterialCommunityIcons
                  ...
                  size={18}
                  color={mode === 'load' ? '#fff' : T.textSecondary}
                />
              ...
              >
                <MaterialCommunityIcons
                  ...
                  size={18}
                  color={mode === 'returns' ? '#fff' : T.textSecondary}
                />
            ...
            <TextInput
              style={s.searchInput}
              placeholder={mode === 'returns' ? 'Rechercher un produit charge' : 'Rechercher un produit'}
            ...
            {loading && products.length === 0 ? (
              ...
              <View style={s.emptyInline}>
                ...
                <Text style={s.emptyInlineTitle}>
                  {mode === 'returns' ? 'Aucun produit retour' : 'Aucun produit trouve'}
                </Text>
                <Text style={s.emptyInlineText}>
                  {mode === 'returns'
                    ? 'Les retours apparaissent seulement pour les produits deja engages dans la session.'
                    : 'Essayez une autre recherche ou actualisez la liste.'}
                </Text>
              </View>
            ) : (
            ...
            >
              {busy ? (
                ...
                <Text style={s.primaryButtonText}>
                  {mode === 'returns' ? 'Valider les retours' : 'Valider le chargement'}
                </Text>
              )}
            </TouchableOpacity>
          </View>
